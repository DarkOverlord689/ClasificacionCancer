...
def predicto(model_name, image_path, age, sex, localization, patient_id, metadata_path='metadatos_T.csv'):
    logging.info(f"Starting prediction for patient_id: {patient_id} using model: {model_name}")
    ...
    try:
        ...
        raise  # Re-lanzar la excepción para manejarla en el lugar de la llamada

    # Obtener la fecha y hora actual para el diagnóstico
    diagnosis_date = datetime.now()

    # Crear directorios para el paciente y las interpretaciones
    try:
        ...
        logging.info(f"Directories created: {patient_dir}, {interpretation_dir}")
    except Exception as e:
    ...
    interpretations = {}
    for model_name, heatmap in result.get('interpretations', {}).items():
        try:
            image_filename = f'interpretation_{model_name}.jpg'
            image_save_path = os.path.join(interpretation_dir, image_filename)
            heatmap.save(image_save_path)

            interpretations[model_name] = image_save_path  # Guarda la ruta de la imagen
            logging.info(f"Saved interpretation image for {model_name} at {image_save_path}.")
        except Exception as e:
            logging.error(f"Error saving interpretation image for {model_name}: {str(e)}")

    result['interpretations'] = interpretations  # Mantén las rutas en el resultado
    logging.info(f"Interpretation images saved: {result['interpretations']}")
