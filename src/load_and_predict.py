...
from src.utils import get_patient_directory, get_interpretation_directory
...
def predicto(model_name, image_path, age, sex, localization, patient_id, metadata_path='metadatos_T.csv'):
    ...
    for model_name, heatmap in result['interpretations'].items():
        try:
            ...
            image_path = os.path.join(interpretation_dir, image_filename)
            heatmap.save(image_path)
            
            interpretations[model_name] = image_path  # Guarda la ruta de la imagen
            
            logging.info(f"Saved interpretation image for {model_name}.")
        except Exception as e:
